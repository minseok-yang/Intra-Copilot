...
// 커넥터 설정입니다. 임베딩 서버는 챗봇의 LLM 서버와 따로 둡니다(사내에서 다른 서버로 열릴 수 있음).
// 색인 자체(노트별 벡터)는 설정이 아니라 플러그인 폴더의 connector-index.bin에 있습니다(connector/connector-index.ts).
export interface ConnectorSettings {
	...
	// 자동 갱신 주기(초). 노트를 고친 뒤 이만큼 지나면 바뀐 노트를 색인합니다. 0이면 자동으로 보내지 않습니다(connector-index.ts의 requestSync).
