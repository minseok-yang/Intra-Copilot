...
export type GeneratorFolderKey = 'templateFolder' | 'outputFolder';

// 제너레이터 폴더(양식·저장)를 [찾기] 창으로 고릅니다. 설정 화면과 제너레이터 화면이 같은 규칙을 쓰도록
// 이 함수 하나로 모았습니다(한쪽만 고치면 두 곳에서 받는 폴더가 달라집니다).
// 경로를 손으로 적게 하지 않는 이유: 오타 하나로 엉뚱한 폴더가 새로 생기고, 그때는 양식이 없는 것처럼
// 보여서 원인을 찾기 어렵습니다. 고를 수 있는 것은 이 볼트 안의 폴더뿐입니다.
export function pickGeneratorFolder(
	plugin: IntraCopilotPlugin,
	key: GeneratorFolderKey,
	onChanged: () => void, // 바꾼 뒤 부를 일(화면 갱신·저장)
): void {
	const strings = plugin.strings();
	const generator = plugin.settings.generator;
	new FolderPickerModal(plugin.app, {
		strings: strings.folders,
		current: generator[key],
		onChoose: (path) => {
			const folder = cleanVaultFolder(path);
			// 쓸 수 없는 폴더는 이유를 알림으로 보여 주고 설정은 그대로 둡니다 — 조용히 다른 폴더로
			// 바꿔 두면 고른 것이 왜 안 먹히는지 알 수 없습니다.
			// 양식 폴더를 볼트 맨 위로 두면 볼트의 모든 노트가 양식 목록에 올라옵니다. 저장 폴더를 볼트 맨 위로
			// 두면 만든 노트가 맨 위에 쌓이고, 양식 폴더와 같으면 만든 노트가 다음부터 양식으로 보입니다.
			let why: string | null = null;
			if (!folder) why = key === 'templateFolder' ? strings.generator.folderRootRejected : strings.generator.outputRootRejected;
			else if (key === 'outputFolder' && folder === cleanVaultFolder(generator.templateFolder)) {
				why = strings.generator.outputSameRejected;
			}
			if (why) {
				new Notice(why);
				return;
			}
			generator[key] = folder;
			onChanged();
		},
	}).open();
}
...
function addFolderSetting(
	containerEl: HTMLElement,
	ctx: SettingsContext,
	options: { name: string; desc: string; key: GeneratorFolderKey; openFailed: string },
): void {
	...
	const showPath = () => pathEl.setText(generator[options.key] || folders.root);
	showPath();
	...
	setting.addButton((button) =>
		button
			...
			.onClick(() =>
				pickGeneratorFolder(ctx.plugin, options.key, () => {
					showPath();
					ctx.saveSoon();
				}),
			),
	...
	setting.addButton((button) =>
		button.setButtonText(folders.openFolder).onClick(async () => {
			const folder = generator[options.key];
