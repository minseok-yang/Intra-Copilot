...
- **챗봇 사이드바** — 리본의 봇 아이콘을 누르면 오른쪽에 열립니다. 사내 LLM 서버(OpenAI 호환 API)와 대화하고, 상단에서 모델을 바로 바꿀 수 있습니다.
- **설정 화면** — "일반" / "LLM 연결" 탭으로 나뉘어 있습니다. 서버 주소·API 키·모델을 지정하고 연결 상태를 확인할 수 있으며, 화면 표시 언어를 한국어/English로 전환할 수 있습니다.
...
src/
  ...
  llm/client.ts    OpenAI 호환 API 호출
  ui/              설정 화면, 챗봇, 문서 뷰어 등 화면 코드
```
